  ...
  return (
    <section className="mx-auto max-w-4xl px-4 py-16 sm:px-6 sm:py-20">
      ...
      <header className="mb-12 border-b border-border/60 pb-8">
        ...
        <p className="mt-4 font-mono text-xs uppercase tracking-widest text-text-muted">{t('terms.lastUpdated')}</p>
      </header>
      <div className="space-y-10">
        {SECTIONS.map((s) => (
          <div key={s}>
            ...
            <p className="mt-3 text-[15px] leading-relaxed text-text-muted">{t(`terms.${s}.body`)}</p>
          </div>
        ))}
      </div>
    </section>
